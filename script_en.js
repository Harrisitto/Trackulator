...
function calculateVel(cadData) {
    ...
    const velData = {
        "Speed:  ": [`${velLineal.toFixed(2)} km/h`, velLineal.toFixed(2)],   // Velocidad lineal
        "Time per lap:  ": [`${velVuelta.toFixed(2)} sec`, velVuelta],                  //Velocidad vuelta
...
function displayCadResults(rel) {
    ...
    if(rel < 0){
        const h1 = document.createElement("p");
        h1.classList.add("results");
        h1.innerText = `Input the speed data, please`;
        divElement.appendChild(h1);
    } else {
        let find = 0;
        let error = 0;
        const h1 = document.createElement("p");
        h1.classList.add("results");
        h1.innerText = `Number --X-- marks the difference between ideal and given gear ratios. Better aproximations give numbers closer to 0.`;
        divElement.appendChild(h1);
    ...
    for(let i = 65; i > 38; i--){
        for(let j = 10; j < 35; j++){
            ...
            if(relij < error){
                const h1 = document.createElement("p");
                h1.classList.add("results");
                h1.innerText = `Big ring: ${i}, casette: ${j}.\n--${relij}--`;
                divElement.appendChild(h1);
                find++;
            }
        }
